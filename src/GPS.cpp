...
void makeConfig() {
    ...
    //GPS_PORT.begin(9600, 134217756U, GPS_RX, GPS_TX); 

    byte packet1[] = {0xB5, 0x62, 0x06, 0x8A, 0x11, 0x00, 0x01, 0x01, 0x00, 0x00, 0x21, 0x00, 0x11, 0x20, 0x08, 0x01, 0x00, 0x52, 0x40, 0x00, 0xC2, 0x01, 0x00, 0x53, 0x16};
    byte packet2[] = {0xB5, 0x62, 0x06, 0x8A, 0x08, 0x00, 0x01, 0x00, 0x20, 0x11, 0x00, 0x21, 0x08, 0x0A, 0xEC};
    byte packet3[] = {0xB5, 0x62, 0x06, 0x8A, 0x09, 0x00, 0x01, 0x00, 0x20, 0x11, 0x00, 0x21, 0x08, 0x0B, 0xF5};

    sendPacket(packet1, sizeof(packet1));

    GPS_PORT.end(); 
    GPS_PORT.begin(115200, 134217756U, GPS_RX, GPS_TX); 
...
void gpsSetup(int a, int b, int c, int d, int e){
  ...
  if (a == 57600) { 
      ...
      sendPacket(packet2, sizeof(packet2));
      GPS_PORT.end(); 
      GPS_PORT.begin(57600, 134217756U, GPS_RX, GPS_TX); 
      delay(100);
  }
